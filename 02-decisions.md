...
Chant phase has no audio at all — it's visual/timer only, the client chants Om themselves. Don't attach any audio file to chant phases.
Pranayama phases speak the technique's name once at the start of the phase — not a looping track, not ambient sound for the whole duration.
Mute setting is scoped, not global. One toggle ("mute technique names") silences only the technique-name audio category. The witness-phase bell and the final ending sound must always play regardless of that toggle — they're not part of what gets muted.
...
D15: Persistent session progress notification with platform-specific countdown

Reason: Practitioners close their eyes or set their phone down during sessions. A persistent notification allows checking progress without unlocking or reopening the app.

Platform Implementation Rules:
1. Android: Uses native `usesChronometer` + `chronometerCountDown` with `when` timestamp set to the phase completion time. The Android OS handles the live-ticking countdown natively without background JS execution.
2. iOS: iOS does not support live-ticking notification clocks without Live Activities / WidgetKit extensions. Therefore, on iOS, the notification updates once per phase change with the current phase name and initial duration.
3. Pause State: When paused, the notification switches from a chronometer countdown to a static paused state (e.g. `[Paused] Kapalbhati - 02:30 remaining`) to prevent false counting down while paused.
4. Dismissal: Automatically dismissed on session completion or manual exit/reset.
5. Permission Request: Requested dynamically on the user's first session start, with a clear explanation, never at app launch.
6. Known Limitation: If Android OS force-kills the background app process, the notification chronometer may continue visually ticking to its scheduled target time even though phase advancement has stopped.
